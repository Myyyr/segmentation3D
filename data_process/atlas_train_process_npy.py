...
def prepare_data(input_folder, output_folder, size, input_channels, target_resolution):
    ...
    for train_test in ['train']:
        ...
        for file in file_list:
            ...
            path = os.path.join(output_folder, str(patient_id)+'.npy')
            inp_targ = np.array([img, mask])
            np.save(path, inp_targ)
...
if __name__ == '__main__':
    ...
    preprocessing_folder += "{}_{}_{}".format(target_size[0],target_size[1],target_size[2])
    if not os.path.isdir(preprocessing_folder):
        os.mkdir(preprocessing_folder)
    # target_size = (256,256,112)
