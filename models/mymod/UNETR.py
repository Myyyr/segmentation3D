...
class UNETR(nn.Module):
    ...
    def __init__(self, filters = [64, 128, 256, 512], d_model=768, input_shape= (512,512,512), patch_size=(16,16,16), skip_idx = [3,6,9,12], n_classes=2, in_channels=1, n_heads=8, bn = True, up_mode='deconv', n_layers=12):
        ...
        self.up_concat3 = nn.ModuleList([UNetConv3D(self.filters[0]*2,self.filters[1], bn=bn), nn.ConvTranspose3d(self.filters[1], self.filters[1], 2, stride = 2,output_padding=1)])
        self.up_concat2 = nn.ModuleList([UNetConv3D(self.filters[1]*2,self.filters[2], bn=bn), nn.ConvTranspose3d(self.filters[2], self.filters[2], 2, stride = 2,output_padding=1)])
        self.up_concat1 = nn.ModuleList([UNetConv3D(self.filters[2]*2,self.filters[3], bn=bn), nn.ConvTranspose3d(self.filters[3], self.filters[3], 2, stride = 2,output_padding=1)])
