...
import json
import os
class TCIAPancreas(Train):

    def __init__(self, expconfig, split = 0):
        super(TCIAPancreas, self).__init__(expconfig)
        ...
        trainDataset = SplitTCIA3DDataset(expconfig, mode="train", randomCrop=None, hasMasks=True, returnOffsets=False, split = split)
        validDataset = SplitTCIA3DDataset(expconfig, mode="validation", randomCrop=None, hasMasks=True, returnOffsets=False, split = split)
